...
def add_new_flight():
    ...
    # Flight Designator: Autogenerated 4 Digit Code
    ...
    while flag:
        ...
        aircraft_model = aircraft_type_records[user_input][1]

        # Flights for aircraft with selected type
...
WHERE aircraft.manufacturer = ? AND aircraft.aircraft_model = ?"
        ...
        flights_with_aircraft_type = select_query(
            cmd, (manufacturer, aircraft_model))
        ...
        for rows in flights_with_aircraft_type:
            ...
            row_arrival = rows[2]
            # If selected times is between row time. (Check departure then arrival)
            if is_between_iso8601(departure_iso8601, row_departure,
                                  row_arrival) or is_between_iso8601(
                                      arrival_iso8601, row_departure,
                                      row_arrival):
                continue
            # If row time is between selected times. (Check departure then arrival)
        ...
        if chosen:
            ...
            print(
                "🛫 No available aircraft of desired type, please select a different aircraft type."
            )
            first_time = False
    ...
    print_color(
        "✈️ The " + manufacturer + " " + aircraft_model +
